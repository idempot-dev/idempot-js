...
test("PostgresIdempotencyStore - lookup with empty store", async (t) => {
  const mockPool = createMockPool([{ rows: [], rowCount: 0 }, { rows: [], rowCount: 0 }]);
  const store = new PostgresIdempotencyStore({
    connectionString: "postgres://localhost/test",
    pool: mockPool
  });
...
test("PostgresIdempotencyStore - lookup returns parsed records", async (t) => {
  const mockPool = createMockPool([
    { rows: [{ key: "test-key", fingerprint: "test-fp", status: "complete", response_status: 200, response_headers: '{"content-type":"application/json"}', response_body: '{"result":"ok"}', expires_at: Date.now() + 60000 }], rowCount: 1 },
    { rows: [{ key: "test-key", fingerprint: "test-fp", status: "complete", response_status: 200, response_headers: '{"content-type":"application/json"}', response_body: '{"result":"ok"}', expires_at: Date.now() + 60000 }], rowCount: 1 }
  ]);
  const store = new PostgresIdempotencyStore({
    connectionString: "postgres://localhost/test",
...
test("PostgresIdempotencyStore - lookup handles missing response", async (t) => {
  const mockPool = createMockPool([
    { rows: [{ key: "test-key", fingerprint: "test-fp", status: "processing", expires_at: Date.now() + 60000 }], rowCount: 1 },
    { rows: [{ key: "test-key", fingerprint: "test-fp", status: "processing", expires_at: Date.now() + 60000 }], rowCount: 1 }
  ]);
  const store = new PostgresIdempotencyStore({
    connectionString: "postgres://localhost/test",
...
test("PostgresIdempotencyStore - complete updates record", async (t) => {
  const mockPool = createMockPool([{ rows: [{ key: "test-key" }], rowCount: 1 }]);
  const store = new PostgresIdempotencyStore({
    connectionString: "postgres://localhost/test",
    pool: mockPool
  });
...
test("PostgresIdempotencyStore - lookup with different key and fingerprint", async (t) => {
  const mockPool = createMockPool([
    ...
    { rows: [{ key: "key-1", fingerprint: "fp-1", status: "processing", expires_at: Date.now() + 60000 }], rowCount: 1 }  // SELECT by fingerprint
  ]);
  const store = new PostgresIdempotencyStore({
    connectionString: "postgres://localhost/test",
    pool: mockPool
...
test("PostgresIdempotencyStore - lookup deletes expired records", async (t) => {
  ...
  t.ok(firstCall.sql.includes("DELETE") && firstCall.sql.includes("expires_at"), "should delete expired");
